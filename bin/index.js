...
class Todo_list {
    ...
    //view todo list
    async view_todo_list() {
        let view = this._all_list;
            ...
            const view_list1 = await inquirer.prompt([
                {
                    ...
                    choices: function () {
                        ...
                        if (match === "autor") {
                            const _autor = view.map((item) => item.autor);
                            return _autor;
                        }
                        else if (match === "date") {
                            const _date = view.map((item) => item.date);
                            return _date;
                        }
                        else if (match === "subject") {
                            const _subject = view.map((item) => item.subject);
                            return _subject;
                        }
                    },
    ...
    async backOrExit() {
        ...
        let userOption = back_exit.main_exit;
        if (userOption === "Main") {
            this.main();
        }
        else {
